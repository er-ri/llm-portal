...
async function onChat() {
  ...
  scrollToBottom();
  prompt.value = ""; // Clear input contents after creating the chat box
  ...
  const openai = new OpenAI({
    apiKey: userStore.userInfo.openaiApiKey,
  ...
  for await (const chunk of completion) {
    ...
    if (chunk.choices[0].finish_reason !== "stop") {
      receivedText = receivedText + chunk.choices[0].delta.content ?? "";
